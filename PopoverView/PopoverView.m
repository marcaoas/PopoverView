...
- (CGSize) screenSize
{
  ...
  if (UIInterfaceOrientationIsLandscape(orientation))
    ...
    -(void)setupLayout:(CGPoint)point inView:(UIView*)view
    {
      ...
      //self.frame = CGRectMake(contentFrame.origin.x - 20, contentFrame.origin.y + 20, contentFrame.size.width, contentFrame.size.height);
      ...
      if (!animated)
        {
        ...
        else
          {
            [UIView animateWithDuration:0.3f animations:^{
              ...
              self.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
              contentView.transform = CGAffineTransformMakeScale(0.1f, 0.1f);
            } completion:^(BOOL finished) {
              [self dismissComplete];
            }];
          }
        ...
        {
          if (([touch.view class] == [UIButton class])) {//change it to your condition
            return NO;
          }
          return YES;
        }
